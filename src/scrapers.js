...
export async function scrapeSavona(context, range, timeout) {
  const page = await context.newPage();
  try {
    await goto(page, URLS.savona, timeout);
    await page.waitForSelector("a.releases-item", { timeout: Math.min(timeout, 15000) });
    const candidates = await page.locator("a.releases-item.releases-item_soon").evaluateAll((anchors) => anchors
      .map((a) => ({
        url: a.href,
...
export async function collectEvents(browser, range, timeout = 35000) {
  ...
  try {
    const jobs = [
      ...
      ["savona", scrapeSavona(context, range, timeout)]
    ];
